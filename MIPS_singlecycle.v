...
// COMPLETE TESTBENCH GENERATED BY AI FOR TESTING THE PROCESSOR 
...
module tb_cpu;
    ...
    task dump_registers;
        begin
            ...
            for (i = 0; i < 32; i = i + 1)
                $display("R[%0d] = %h", i, dut.RF.regs[i]);
        end
    endtask

    task dump_memory(input integer start, input integer count);
        begin
            $display("---- DATA MEMORY ----");
            for (i = start; i < start + count; i = i + 1)
                $display("MEM[%0d] = %h", i, dut.DM.mem[i]);
        end
    endtask
    ...
    task check_reg(
        ...
        begin
            if (dut.RF.regs[regno] !== expected)
                $display("❌ FAIL: R[%0d] = %h (expected %h)",
                         regno, dut.RF.regs[regno], expected);
            else
                $display("✅ PASS: R[%0d] = %h", regno, expected);
        end
    endtask
    ...
    task check_mem(
        ...
        begin
            if (dut.DM.mem[addr] !== expected)
                $display("❌ FAIL: MEM[%0d] = %h (expected %h)",
                         addr, dut.DM.mem[addr], expected);
            else
    ...
    initial begin
        ...
        check_reg(10, 32'h0000000A);   // example
        check_mem(2, 32'h0000000A);
